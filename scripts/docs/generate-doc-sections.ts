...
export const SECTIONS: DocSection[] = [
  ...
  },
  {
    ...
    output: 'docs/docfx/docs/configuration',
    hasIndex: false,
    order: [
      'index.md',
      'season-and-applications.md',
      'outages.md',
      'household-view.md',
      'enrollment-checker.md',
      'identity-and-proofing.md',
      'addresses-and-messaging.md',
      'operations.md',
      'non-production.md',
    ],
    parent: 'docs/docfx/docs',
    nav: ['Customizing', 'Configure features and settings'],
  },
  {
 ...
 */
export function buildParentToc(groups: DocGroup[], extras: { name: string; href: string }[] = []): string {
 ...
 *
 * The changelog link points at the file's commit history on GitHub, which is
 * the page's own change history without a changelog to maintain by hand.
 */
export function stampLastUpdated(markdown: string, iso: string, sourcePath: string): string {
  ...
  const history = `${REPO_URL}/commits/${REPO_BRANCH}/${sourcePath}`;
  const stamp =
    `\n<p class="doc-meta">Last updated <time datetime="${day}">${formatStampDate(iso)}</time>` +
    ` &middot; <a href="${source}">View source</a>` +
    ` &middot; <a href="${history}">View changelog</a></p>\n`;
...
function main(): void {
  ...
  for (const [parent, entries] of grouped) {
    ...
    const extras = PARENT_EXTRAS[parent] ?? [];
