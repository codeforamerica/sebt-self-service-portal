...
- **i18n integrity is gated at commit and build.** `pnpm lint:i18n` (in `src/SEBT.Portal.Web/`) cross-checks every `t('key')` call against the generated JSON and fails on missing/empty values that aren't in `docs/i18n-audit-baseline.json`. Runs in pre-commit (frontend or content/ changes) and `prebuild`. To clear an entry: fix the sheet, re-run `pnpm copy:generate`, then `pnpm lint:i18n:update-baseline` to write a smaller baseline. See [docs/i18n-audit.md](./docs/i18n-audit.md).
