...
3. **General transform-expression engine (JSONata, JUST, JsonLogic)** — replace the primitive catalog with one expression evaluator.
   🟡 Roughly 1,200 LOC smaller. But it trades load-time fail-fast validation for silent wrong answers at runtime — a bad expression maps garbage instead of refusing to start. Revisit at state #3 if the catalog sprawls.
