...
3. **Evaluation** in `SelfServiceEvaluator`, which reads `IOptionsSnapshot<SelfServiceRulesSettings>` and produces `HouseholdAllowedActions` (aggregate) and `CaseAllowedActions` (per-case). The evaluator injects into every write handler; handlers return `Result.Forbidden` (HTTP 403) on denial. The API response includes the same allowedActions structures so the frontend can gate UI accordingly.
...
- **Policy tuning is a config edit.** No code change, no deploy pipeline, no migration. `IOptionsSnapshot` means the next request picks up changes without even an API restart in some paths.
