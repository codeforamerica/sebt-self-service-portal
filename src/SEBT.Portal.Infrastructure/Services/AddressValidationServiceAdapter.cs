    ...
    public async Task<AddressValidationResult> ValidateAsync(Address address, CancellationToken cancellationToken = default)
    {
        ...
        return result switch
        {
            ...
            ValidationFailedResult<AddressUpdateSuccess> vf =>
                AddressValidationResult.Invalid(string.Join(" ", vf.Errors.Select(e => e.Message)), "validation_failed"),
            ...
            DependencyFailedResult<AddressUpdateSuccess> df =>
                AddressValidationResult.Invalid(df.Message, "dependency_failed"),
            _ => AddressValidationResult.Invalid("Address verification failed.", "unknown")
        };
    }
}
