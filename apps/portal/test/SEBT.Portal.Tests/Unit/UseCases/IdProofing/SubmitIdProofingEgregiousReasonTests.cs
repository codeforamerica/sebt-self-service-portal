...
public class SubmitIdProofingEgregiousReasonTests
{
    ...
    [Fact]
    public async Task Handle_ShouldReturnEgregiousFailedOffboardingReason_WhenFailedOutcomeHasEgregiousReasonCode()
    {
        var handler = CreateHandler();
        var command = new SubmitIdProofingCommand
        {
        ...
        await userRepository.Received(1)
            .UpdateUserAsync(Arg.Is<User>(u => u.IdProofingAttemptCount == 1), Arg.Any<CancellationToken>());
        await challengeRepository.DidNotReceive()
            .CreateAsync(Arg.Any<DocVerificationChallenge>(), Arg.Any<CancellationToken>());
    }
}
